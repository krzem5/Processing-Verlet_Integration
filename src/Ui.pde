class Ui{
	...
	void draw(){
		...
		this.write_text(text,10,10+textAscent(),false);
		...
		this.write_text(text,width-10,10+textAscent(),true);
	}



	private void write_text(String text,float x,float y,boolean align_to_right){
		textFont(this._ui_font);
		noStroke();
		fill(0xb4ffffff);
		for (String line:text.split("\n")){
			...
			float line_x=(align_to_right?x-line_width:x);
			i=0;
			while (i<line.length()){
				char c=line.charAt(i);
				if (c=='<'){
